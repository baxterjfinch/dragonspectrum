...
class CounterShardConfig(ndb.Model):
    ...
    def all_keys(cls, name):
        ...
        return [ndb.Key(CounterShard, shard_key_string)
...
def get_count(name):
    ...
    if total is None:
        total = 0
        all_keys = CounterShardConfig.all_keys(name)
...
def _increment(name, num_shards):
    ...
    counter = CounterShard.get_by_id(shard_key_string)
    if counter is None:
        counter = CounterShard(id=shard_key_string)
    counter.count += 1
    counter.put()
