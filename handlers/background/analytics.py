...
class RunAnalyticsHandler(webapp2.RequestHandler):
    ...
    def _finallize_anastat(self, anastat):
        # Clear this out just in case this got repeated
...
class RunHourlyAnalyticsHandler(RunAnalyticsHandler):
    ...
    def _record_project_ana_action(self, ana):
        ...
        aid =  ana.artifact.id() + '-' + self.end_time.strftime('%Y-%m-%d-%H-%M')
        anastat = self.art_ana_stats.get(aid, None)

        if not anastat:
            in_art_ana_state = False
            anastat = ConceptAnalyticsHourlyStat.get_by_id(aid)

            if not anastat:
                anastat = ConceptAnalyticsHourlyStat.new(id=aid)
        ...
        if not in_art_ana_state:
            self.art_ana_stats[aid] = anastat
    ...
    def _record_concept_ana_action(self, ana):
        ...
        if not anastat:
            ...
            if not anastat:
                anastat = ConceptAnalyticsHourlyStat.new(aid)
                anastat.artifact = ana.artifact
                anastat.start_time = self.start_time
                anastat.end_time = self.end_time
                anastat.entity = ana.artifact
    ...
    def _record_user_ana_action(self, ana):
        ...
        aid = user_key.id() + '-' + self.end_time.strftime('%Y-%m-%d-%H-%M')

        anastat = self.user_ana_stats.get(aid, None)
        if not anastat:
            in_user_ana_state = False
            anastat = UserAnalyticsHourlyStat.get_by_id(aid)

            if not anastat:
                anastat = UserAnalyticsHourlyStat.new(id=aid)
                anastat.start_time = self.start_time
                anastat.end_time = self.end_time
        ...
        if not in_user_ana_state:
            self.user_ana_stats[aid] = anastat
    ...
    def _process_hour_block(self, anas):
        ...
        for ana in anas:
            if ana.artifact_organization:
                aid = ana.artifact_organization.id() + '-' + self.end_time.strftime('%Y-%m-%d-%H-%M')
